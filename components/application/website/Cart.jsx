  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger className='relative'>
        <BsCart2 className='text-gray-500 hover:text-primary cursor-pointer' size={25} />
        <span className='absolute flex items-center justify-center w-4 h-4 text-white bg-red-500 text-xs rounded-full -right-2 -top-1'>{cart.count}</span>
      </SheetTrigger>
      <SheetContent className='sm:max-w-[450px] w-full'>
        ...
        <div className='h-[calc(100vh-40px)] pb-10'>
          <div className='h-[calc(100%-110px)] overflow-auto px-2'>
            {
              cart.count === 0 && <div className='h-full flex items-center justify-center text-xl font-semibold'>Your cart is empty.</div>
            }
            {
                ...
                <div key={product.variantId} className='flex justify-between items-center gap-5 mb-4 border-b pb-4'>
                  ...
                  <div>
                    <button type='button' className='text-red-500 underline underline-offset-1 mb-2 cursor-pointer'
                      onClick={() => dispatch(removeFromCart({ productId: product.productId, variantId: product.variantId }))}>Remove</button>
          ...
          <div className='h-28 border-t pt-5 px-2'>
            ...
            <div className='flex justify-between gap-5 mt-3'>
              <Button type='button' variant={'secondary'} asChild className='w-[200px]' onClick={() => setOpen(false)}>
                <Link href={WEBSITE_CART}>View Cart
                </Link>
              </Button>
