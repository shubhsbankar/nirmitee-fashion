...
export async function getProductDetailsBySlug({ slug, size, color }) {
  ...
  if (!variant) {
    // Don't treat "no variants" as "product not found".
    // Log with enough context to debug data issues in Vercel logs.
    console.warn('[product-details] variant not found', {
      slug: normalizedSlug,
  ...
  const getColor = variant
    ? await ProductVariantModel.distinct('color', {
        product: data._id,
        deletedAt: null,
      })
    : [];
